...
class RestaurantOrderManagement:
    def __init__(self, root):
        ...
        for i, (items, price) in enumerate(self.menu_items.items(), start=1):
            label = ttk.Label(frame,
                               text=f"{items} (${price}):",
                               font=("Ariel", 12))
            label.grid(row=i, coloumn=0, padx=10, pady=5)
            self.menu_labels[items] = label
            quantity_entry = ttk.Entry(frame, width=5)
            quantity_entry.grid(row=i, coloumn=1, padx=10,pady=5)
    ...
    def setup_bg(self, root):
     ...
     canvas.create_image(0, 0, anchor=tk.NW, image="C:/Users/hp/Desktop/Python/GUI-based/restaurant.png")
